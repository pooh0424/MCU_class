//
// RTC_LCD_Clock : RTC update a clock on LCD
//
// EVB : Nu-LB-NUC140
// MCU : NUC140VE3CN
#include <stdio.h>
#include <math.h>
#include "NUC100Series.h"
#include "MCU_init.h"
#include "SYS_init.h"
#include "LCD.h"
#include "Draw2D.h"

#define PI 3.1415926
#define X0 64
#define Y0 32
#define R  20

volatile uint8_t old_xs, old_ys, new_xs, new_ys,old_xm, old_ym, new_xm, new_ym;
volatile uint16_t minute =0;

unsigned char bmp_Clock[64*8]= 
{
	
};

uint8_t RTC_Flag;
uint16_t theta;	

void RTC_IRQHandler(void)
{
	S_RTC_TIME_DATA_T sCurTime;
    RTC_GetDateAndTime(&sCurTime);	// get current time
    theta = sCurTime.u32Second *6;

    //------------drawing clock pointer---------------
    if (theta>=360) theta=0;
	
    new_xs = X0 + (R-2) * sin(theta * PI/180);
    new_ys = Y0 - (R-2) * cos(theta * PI/180);
	
    draw_Line(old_xs,old_ys,X0,Y0,BG_COLOR, BG_COLOR);
    draw_Line(new_xs,new_ys,X0,Y0,FG_COLOR, BG_COLOR);
	
    old_xs = new_xs;
    old_ys = new_ys;	
    //------------------------------------------------
		if(minute==59){
				minute=0;
		}
		else{
				minute++;
		}
		if(minute%10==0){
			theta = minute *6;
			new_xm = X0 + (R-6) * sin(theta * PI/180);
			new_ym = Y0 - (R-6) * cos(theta * PI/180);
		
			draw_Line(old_xm,old_ym,X0,Y0,BG_COLOR, BG_COLOR);
			draw_Line(new_xm,new_ym,X0,Y0,FG_COLOR, BG_COLOR);
			old_xm = new_xm;
			old_ym = new_ym;	
		}
		else{
			draw_Line(old_xm,old_ym,X0,Y0,FG_COLOR, BG_COLOR);
		}

    RTC_CLEAR_TICK_INT_FLAG();	
}

void Delay(uint32_t ucnt)
{
    volatile uint32_t i = ucnt;
    while (i--);
}

void Init_RTC(void)
{
  S_RTC_TIME_DATA_T sInitTime;

    // Time Setting
    sInitTime.u32Year       = 2015;
    sInitTime.u32Month      = 11;
    sInitTime.u32Day        = 29;
    sInitTime.u32Hour       = 9;
    sInitTime.u32Minute     = 15;
    sInitTime.u32Second     = 0;
    sInitTime.u32DayOfWeek  = RTC_SUNDAY;
    sInitTime.u32TimeScale  = RTC_CLOCK_24;

    RTC_Open(&sInitTime);
    RTC_SetTickPeriod(RTC_TICK_1_SEC);
    RTC_EnableInt(RTC_RIER_TIER_Msk);
    NVIC_EnableIRQ(RTC_IRQn);		  
}

int32_t main(void)
{
    SYS_Init();

    init_LCD();
    clear_LCD();
	
    draw_Bmp64x64(32, 0, FG_COLOR, BG_COLOR, bmp_Clock);
	
    old_xs = X0 + (R-2) * sin(59*6 * PI/180);
    old_ys = Y0 - (R-2) * cos(59*6 * PI/180);
    old_xm = X0 + (R-6) * sin(59*6 * PI/180);
    old_ym = Y0 - (R-6) * cos(59*6 * PI/180);

    Init_RTC();

    while(1);
}
